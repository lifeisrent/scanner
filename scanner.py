...
def scan_image(path: Path, *, min_confidence: float = 50.0) -> List[DetectedText]:
...
def scan_inputs(inputs: Iterable[Path], *, min_confidence: float = 50.0) -> List[DetectedText]:
        ...
        if input_path.is_dir():
            for child in sorted(input_path.iterdir()):
                if child.is_file() and _is_image_file(child):
                    results.extend(scan_image(child, min_confidence=min_confidence))
        elif input_path.is_file() and _is_image_file(input_path):
            results.extend(scan_image(input_path, min_confidence=min_confidence))
        else:
...
def scan_to_spreadsheet(inputs: Iterable[Path], output_path: Path, *, min_confidence: float = 50.0) -> Path:
    """High-level helper: scan images and export detections to a spreadsheet."""

    detections = scan_inputs(inputs, min_confidence=min_confidence)
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    detections = scan_inputs(args.inputs, min_confidence=args.min_confidence)
